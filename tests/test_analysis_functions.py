...
def test_get_atom_densities_top_polrot():
    ...
    try:
        ...
        resonance_2 = experiment_param_values_polrot["state_2_unitarity_res_freq_MHz"]
        resonance_3 = experiment_param_values_polrot["state_3_unitarity_res_freq_MHz"]
        ...
        detuning_A3 = imaging_multiplier * (frequency_A - resonance_3) + hf_lock_offset 
        detuning_B3 = imaging_multiplier * (frequency_B - resonance_3) + hf_lock_offset 
        detunings = np.array([
            [detuning_A1, detuning_A3],
            [detuning_B1, detuning_B3]
        ])
        ...
        single_pixel_polrot_density_1, single_pixel_polrot_density_3 = single_pixel_polrot_return_arrays
        ...
        expected_polrot_density_3 = generate_default_image_density_pattern(crop_to_roi = True, density_value = single_pixel_polrot_density_3[0])
        polrot_density_1, polrot_density_3 = analysis_functions.get_atom_densities_top_polrot(my_measurement, my_run, first_state_index = 1, 
                                                                                              second_state_index = 3, b_field_condition = "unitarity", 
                                                                                              use_saturation = False)
        
        #Rtol is necessary because of the rounding in image generation
        assert np.allclose(polrot_density_1, expected_polrot_density_1, rtol = 1e-3)
        assert np.allclose(polrot_density_3, expected_polrot_density_3, rtol = 1e-3)
        # 
        #For this result, check also that the constrained problem is well behaved
        #Constrain that state 2 is 0 and check that we get the same results
        constraint = np.array([0, 0])

        (polrot_density_1_constrained, polrot_density_3_constrained, 
        polrot_density_2_constrained) = analysis_functions.get_atom_densities_top_polrot(my_measurement, my_run, first_state_index = 1, 
                                                    second_state_index = 3, b_field_condition = "unitarity", use_saturation = False, 
                                                    constrained_state_index = 2, constraint = constraint)
        
        
        assert np.allclose(polrot_density_1_constrained, expected_polrot_density_1, rtol = 1e-3) 
        assert np.allclose(polrot_density_3_constrained, expected_polrot_density_3, rtol = 1e-3)
        assert np.allclose(polrot_density_2_constrained, 0.0, rtol = 1e-3)
